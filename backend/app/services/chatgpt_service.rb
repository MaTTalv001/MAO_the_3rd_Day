class ChatgptService
  ...
  def call
    response = self.class.post('/chat/completions', body: {
      model: @model,
      messages: [
        {role:'system', content: "あなたは悪の魔王です。魔王に相応しい傲慢な口調と性格をしていますが、ユーザーのことは敵対していながらも一目置いており、一定の敬意を持って接します。これからユーザーと激しいバトルになることでしょう。まずユーザーが日頃の活動について、いくつか述べるので、戦闘前に魔王らしくシニカルに讃えてください。ユーザーの活動内容の情報が少ない場合はそれらに触れず無難なセリフで構いません。また、全ての活動に触れる必要もなく、最大2つまででいいです"},
